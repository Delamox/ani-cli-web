...
nth() {
    ...
    # INSERT ANI-CLI-SERVER
    # cut -c 3-
    echo "$(printf "%s" "$stdin" | cut -f1,3 | tr '\t' ' ' | sed -E 's/^/acs: /')" >&2
    exit 0 
    # /INSERT ANI-CLI-SERVER 
    line=$(printf "%s" "$stdin" | cut -f1,3 | tr '\t' ' ' | rofi -sort -dmenu -i -width 1500 -p "$prompt" | cut -d " " -f 1)
...
play_episode() {
    ...
    echo "$(printf "%s" "$episode" | sed -E 's/^/acs: /')" >&2
    exit 0
...
case "$search" in
    ...
    *)
        if [ "$use_external_menu" = "0" ]; then
            ...
            #DISABLE ANI-CLI-SERVER [ -z "$query" ] && query=$(printf "" | external_menu "" "Search anime: ")
            [ -z "$query" ] && exit 1
        fi
